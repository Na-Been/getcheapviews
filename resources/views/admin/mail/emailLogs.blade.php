    ...
    <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
        <table class="table table-report -mt-2" id="example" style="width:100%">
            ...
            <tr>
                ...
                <th class="text-center whitespace-no-wrap">EMAIL SUBJECT</th>
                <th class="text-center whitespace-no-wrap">SEND DATE</th>
                <th class="text-center whitespace-no-wrap">SENT TO</th>
            ...
            @foreach($emails as $email)
                <tr class="intro-x">
                    ...
                    <td class="text-center">{{$email->created_at->format('Y-m-d')}}</td>
                    <td class="text-center">{{$email->to}}</td>
                    <td class="text-center">{{$email->user->id}}</td>

                    <td class="table-report__action w-56">
                        <div class="flex justify-center items-center">
                            <a class="flex items-center mr-3" href="{{route('view.send.email',$email->id)}}"> <i
                                    data-feather="eye"
                                    class="w-4 h-4 mr-1"></i> View
                            </a>
                            <a class="flex items-center text-theme-6 deleteMail" href="javascript:;" data-toggle="modal"
                               data-value="{{$email->id}}" data-target="#delete-confirmation-modal"> <i
